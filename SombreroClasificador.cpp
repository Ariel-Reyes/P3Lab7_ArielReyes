...
ClaseHogwarts* SombreroClasificador::clasificar_magos_nuevos(vector<Mago*> ma,int year) {
	...
	for(int i=0; i<ma.size(); i++) {
		...
		if(mg) {
			...
			prom_mg = (mg->get_valentia() + mg->get_atrevimiento())/2;
	...
	cout<<"...........Cualidades de mago............."<<endl;
		cout<<"Cualidades Gryffindor: "<<prom_mg<<endl;
		cout<<"Cualidades Ravenclaw: "<<prom_mr<<endl;
		cout<<"Cualidades Slytherin: "<<prom_ms<<endl;
		cout<<"Cualidades Hufflepuff: "<<prom_mh<<endl;
...
void SombreroClasificador::imprimir_casa(vector<Mago*> ma) {
	for(int i=0; i<ma.size(); i++) {
